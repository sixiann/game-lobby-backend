...
current_lobby_id = (
    1  # Sets the lobby_id of new lobbies. Using simple integer for easier testing
)
...
def start_game(lobby_id):
    send(
        {"message": f"Max {max_players} players reached. Game is beginning"},
        to=lobby_id,
    )
...
def create_lobby(data):
    ...
    if players[player_id]:
        send(
            {"error": f"Player {player_id} is already in lobby {players[player_id]}"},
            to=request.sid,
        )
        return
    ...
    lobbies[lobby_id] = {
        "players": [
            player_id
        ],  # initialize list of players with player_id who created it
...
def leave_lobby(data):
    ...
    if players[player_id] != lobby_id:
        ...
        send(
            {"error": f"Player {player_id} is not in lobby {lobby_id}"}, to=request.sid
        )
        return
